...
def visualize_agent_learning_curve(dataset_name, total_time_budget, df, alc, visualizations_dir):
    """
    ...
    plt.step(df['total_time_spent'], df['best_test_score_so_far'], where='post', label = dataset_name)
    plt.scatter(df['total_time_spent'], df['best_test_score_so_far'], s=8, marker="D")
    L=plt.legend()
    L.get_texts()[0].set_text('ALC = ' + str(np.around(alc, decimals=2)))
    plt.ylim(0, 1.05)
    plt.ylabel('test score')
    plt.fill_between(df['total_time_spent'], df['best_test_score_so_far'], step="post", alpha=0.4, color='dodgerblue')
    plt.grid()
    ...
    if normalize_t:
        ...
        plt.text(1.0, df.iloc[-1]['best_test_score_so_far'], str(df.iloc[-1]['best_test_score_so_far']), fontdict=None)
    else:
        plt.xlim(0, total_time_budget)
        plt.xlabel('time (s)')
        plt.text(df['total_time_spent'].max(), df.iloc[-1]['best_test_score_so_far'], str(df.iloc[-1]['best_test_score_so_far']), fontdict=None)

    #=== Save figure and clear the plot
    plt.savefig(visualizations_dir + '/' + dataset_name + ".png", dpi=120, format='png', bbox_inches='tight')
    plt.cla()
    plt.clf()
...
def reveal_score_on_the_test_set(env, dataset_name, df):
    ...
    >>> reveal_score_on_the_test_set(env, dataset_name, df)
             algo_index  algo_time_spent  total_time_spent  score   best_test_score_so_far
    ...
    for i, row in df.iterrows():
        ...
        best_test_score_so_far = max(best_test_score_so_far, score)

        #=== Update the test scores in the dataframe
        df.at[i,'test_score'] = score
        df.at[i,'best_test_score_so_far'] = best_test_score_so_far
    return df
...
def compute_ALC(df, total_time_budget):
    ...
    for i in range(len(df)):
        if i==0:
            if normalize_t:
                alc += df.iloc[i]['best_test_score_so_far'] * (1-df.iloc[i]['total_time_spent'])
            else:
                alc += df.iloc[i]['best_test_score_so_far'] * (total_time_budget-df.iloc[i]['total_time_spent'])
        elif i>0:
            if normalize_t:
                alc += (df.iloc[i]['best_test_score_so_far']-df.iloc[i-1]['best_test_score_so_far']) * (1-df.iloc[i]['total_time_spent'])
            else:
                alc += (df.iloc[i]['best_test_score_so_far']-df.iloc[i-1]['best_test_score_so_far']) * (total_time_budget-df.iloc[i]['total_time_spent'])
    return alc
...
if __name__ == "__main__":
    ...
    for run_number in range(number_of_runs):
        run_output_from_ingestion_program_dir = os.path.join(output_from_ingestion_program_dir, 'run_' + str(run_number))
        visualizations_dir = os.path.join(output_visualizations_dir, 'run_' + str(run_number))
        ...
        for dataset_name in list_datasets:
            ...
            try:
                ...
                df = pd.read_csv(output_file, names=['algo_index', 'algo_time_spent', 'total_time_spent'],
                                dtype={'algo_index': str, 'algo_time_spent': float, 'total_time_spent': float})

                #=== Update the dataframe with scores on the learning curve on the validation set
                updated_df = reveal_score_on_the_validation_set(env, dataset_name, df)

                #=== Update the dataframe with scores on the learning curve on the test set
                updated_df = reveal_score_on_the_test_set(env, dataset_name, updated_df)
                ...
                #=== Get the final score
                final_score = round(updated_df.iloc[-1]['test_score'], 2)

                #=== Compute ALC
                alc = round(compute_ALC(updated_df, total_time_budget),2)

                updated_df.to_csv(os.path.join(output_dir, 'run_' + str(run_number), dataset_name + '.csv'), index=False)

                #=== Visualization
                visualize_agent_learning_curve(dataset_name, total_time_budget, updated_df, alc, visualizations_dir)
        ...
        if len(list_final_score)!=0:
            average_final_score[run_number] = round(sum(list_final_score) / len(list_datasets), 2)
        if len(list_alc)!=0:
            average_alc[run_number] = round(sum(list_alc) / len(list_datasets), 2)

        #=== Write alc score of each dataset for this run
        dataset_alc = dataset_alc.append({'dataset': 'AVERAGE', 'alc':average_alc[run_number]}, ignore_index=True)
        dataset_alc.to_csv(os.path.join(output_dir, 'RUN_' + str(run_number) + '_RESULT_DATASET_ALC.csv'), index=False)
        list_dataset_alc[run_number] = dataset_alc
        
    # Get best run
    worst_index = np.argmin(average_alc)

    #=== Write scores.html
    write_scores_html(output_dir, os.path.join(output_visualizations_dir, 'run_' + str(worst_index)))
