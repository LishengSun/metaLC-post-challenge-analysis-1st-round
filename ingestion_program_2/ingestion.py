...
def meta_testing(trained_agent, D_te, run_number):
    """
...
if __name__ == "__main__":
    ...
    for run_number in range(number_of_runs):
        ...
        run_output = os.path.join(output_dir, 'run_' + str(run_number))
        if not os.path.exists(run_output):
            os.makedirs(run_output)

        #=== Init a meta-learning environment
        env = Meta_Learning_Environment(validation_data_dir, test_data_dir, meta_features_dir, algorithms_meta_features_dir, run_output, run_number)
        ...
        for D_tr, D_te in kf.split(list_datasets):
            ...
            #=== META-TESTING
            meta_testing(trained_agent, D_te, run_number)
