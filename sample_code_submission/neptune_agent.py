...
class Agent(object):
    ...
    def meta_train(self, datasets_meta_features, algorithms_meta_features,
        ...
        # print('====>meta_train test', x1_data.shape, y1_data.shape)
        # loss_sum = 0.0
        # scores_list = []
        # for i in range(y1_data.shape[0]):
        #     if i % 20 == 0:
        #         print('')
        #         print('====> meta train test', ds_name_list[i//20])
        #     score = self.trainer.eval_net(self.net1, x1_data[i])
        #     score = [round(x, 4) for x in score.tolist()]
        #     ts_label = self.trainer.eval_net(self.net2, x2_data[i])
        #     ts_label = np.argmax(ts_label)
        #     print('====> x', x_data[i].tolist())
        #     print('====> y', i % 20, y_data[i].tolist())
        #     print('====> s', i % 20, score)
        #     print('====> ts', i % 20, ts_label)
    ...
    def suggest(self, observation):
        if observation is not None:
            ...
            # if R_validation_C_A < 0.0 and len(self.best_algo_list) > 1:
            #     self.best_algo_list.remove(A)
            #     self.algo_index = 0
